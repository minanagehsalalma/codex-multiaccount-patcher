...
echo "Cirrus latest Codex ref: $latest_ref"
echo "Cirrus latest Codex version: $latest_version"

rm -rf upstream

git clone --depth 1 --branch "$latest_ref" https://github.com/openai/codex.git upstream

node ./scripts/apply-maintained-patch.mjs --upstream-root ./upstream --json > ./dist-cirrus-patch-report.json
